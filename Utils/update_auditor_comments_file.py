...
class UpdateJsonFile:
	...
	def updateAuditorComments(self):
		...
		if isfile(self.auditor_comments_file):
			with open(self.auditor_comments_file, 'r') as ac:
				auditor_comments = json.loads(ac.read())
				vulnerable_jars = auditor_comments.keys()
		else:
			vulnerable_jars, auditor_comments = [], {}
				...
				if jar not in vulnerable_jars:
					if sheet_data.loc[i, 'CVE'].lower() != "no vulnerability":
						issue = self.jar_template.format(
							...
							Comment=sheet_data.loc[i, 'Auditor Comment']
						)
						auditor_comments.update(json.loads(issue))
						print(issue)
				else:
					jar_data = auditor_comments.get(jar)
					...
					if cve.lower() != "no vulnerability":
						...
						auditor_comments[jar] = jar_data
			with open(self.auditor_comments_file, 'w') as acb:
				acb.write(json.dumps(auditor_comments, indent=4))
